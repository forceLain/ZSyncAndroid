...
int connect_to(const char *node, const char *service) {
    ...
    if ((rc = getaddrinfo(node, "80", &hint, &ai)) != 0) {
        perror(node);
        return -1;
    }
    else {
...
FILE *http_get_stream(int fd, int *code) {
    ...
    char* fgets_result = fgets(buf, sizeof(buf), f);
    int memcmp_result = memcmp(buf, "HTTP/1", 6);
    p = strchr(buf, ' ');
    ...
    for (; allow_redirects-- && url && !f;) {
        ...
        {   /* Connect */
            ...
            {   /* Compose request */
                ...
                int sent = send(sfd, buf, strlen(buf), 0);
                if (sent == -1) {
                    perror("sendmsg");
                    close(sfd);
                    break;
            ...
            if (code == 301 || code == 302 || code == 307) {
            	...
            	//if (fname){
            		g = fopen("/storage/sdcard0/tempfile.zzz", "w+");
            	//} else {
            	//	g = tmpfile();
            	//}
